...
async function aaiStartTranscript(audioUrl) {
  const body = { audio_url: audioUrl, punctuate: true, format_text: true };
...
async function aaiPollTranscript(id, timeoutMs = 180000) {
  ...
  while (true) {
    const res = await fetch(`${AAI_BASE}/transcript/${id}`, { headers: { authorization: AAI_KEY } });
...
async function main() {
  const local = "./test-voice.wav"; // converted audio file
